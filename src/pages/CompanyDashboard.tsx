...
export const CompanyDashboard = () => {
  ...
  useEffect(() => {
    if (user && user.userType === 'company' && user.isActive) {
      fetchOrders();
      fetchAddresses();
      fetchProvinces();
    } else if (user && user.userType === 'company' && !user.isActive) {
      toast.error('Your company account is awaiting admin approval.');
  ...
  if (!user || user.userType !== 'company') {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p>Access Denied. This dashboard is for company users only.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-8">
        ...
        <p className="text-gray-600 mb-8">Manage your company orders and addresses here.</p>
        ...
        {activeTab === 'orders' && (
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Your Company Orders</h2>
            {loadingOrders ? (
