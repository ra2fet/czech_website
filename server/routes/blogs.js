...
router.get('/', (req, res) => {
  ...
  db.query(
    `SELECT b.id, bt.title, bt.content, bt.excerpt, b.image_url, b.created_at, b.updated_at
     FROM blogs b
...
router.post('/', authenticateToken, adminProtect, async (req, res) => {
  ...
  try {
    ...
    const [blogResult] = await db.promise().query(
      'INSERT INTO blogs (image_url) VALUES (?)',
      [image_url || null]
    );
...
router.put('/:id', authenticateToken, adminProtect, async (req, res) => {
  ...
  try {
    ...
    const [updateBlogResult] = await db.promise().query(
      'UPDATE blogs SET image_url = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [image_url || null, id]
    );
